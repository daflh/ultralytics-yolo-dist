    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file, encoding="utf-8") as f:
                ...
                lb = [x[:-3]+[x[-1]] for x in lb] # MODIFIED: SO THAT IT WORKS WITH MY LABEL FORMAT
                if any(len(x) > 6 for x in lb) and (not keypoint):  # is segment
                    classes = np.array([x[0] for x in lb], dtype=np.float32)
                    segments = [np.array(x[1:-1], dtype=np.float32).reshape(-1, 2) for x in lb]  # (cls, xy1...)
                    dist = np.array([x[-1] for x in lb], dtype=np.float32)
                    lb = np.concatenate((classes.reshape(-1, 1), segments2boxes(segments), dist.reshape(-1, 1)), 1)  # (cls, xywh)
